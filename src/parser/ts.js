...
class TS {
  ...
  static PMT(buffer, it) {
    ...
    TS.PMTSpace = TS.PMTSpace.concat(
      list.map(item => {
        return {
          pid: item.pid,
          es: item.es,
  ...
  static Merge(buffer, ts) {
    let length = buffer.length - buffer.position,
      data,
      offset = length
    ts.forEach(item => {
